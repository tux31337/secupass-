...
- UI 화면/컴포넌트 변경 전에는 `docs/DESIGN.md`를 확인하고, iOS/Android 키보드 동작, Safe Area, 터치 타깃, 작은 화면 텍스트 겹침 기준을 반영한다.
- 입력 화면은 키보드가 열린 상태에서도 입력창과 핵심 액션이 가려지지 않아야 하며, 이를 고려하지 않은 UI 변경은 완료로 보지 않는다.

## 개발 프로세스
- CRITICAL: 새 기능 구현 시 반드시 테스트를 먼저 작성하고, 테스트가 통과하는 구현을 작성할 것(TDD).
- 구현 전 `docs/PRD.md`, `docs/ARCHITECTURE.md`, `docs/ADR.md`의 결정과 충돌하지 않는지 확인한다.
- UI 변경 전 `docs/DESIGN.md`의 검증 항목을 확인하고, 작은 화면/키보드/iOS/Android 차이에 대한 검증 시나리오를 먼저 정의한다.
